...
public enum OperatorManager
{
    ...
    public void reload( DataSource ds )
            throws SQLException
    {
    ...
    public void reload( Connection con )
            throws SQLException
    {
        try( Statement s = con.createStatement() )
        {
            Map<Integer, Operator> ops = SQL.stream( s.executeQuery(
                    "SELECT * FROM rtppm.operator" ),
                                                     Operator.fromSQL
            ).
                    collect( Collectors.toMap( Operator::getId, Function.
                                               identity() ) );
