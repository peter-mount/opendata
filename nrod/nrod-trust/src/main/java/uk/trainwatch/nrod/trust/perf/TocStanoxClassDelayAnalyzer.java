...
/**
 * Analyses the performance of trains arriving, departing or passing a stanox breaking them down to individual train
 * class.
 * <p>
 * @author Peter T Mount
 */
public class TocStanoxClassDelayAnalyzer
        extends BaseDelayAnalyzer
{
    ...
    private final Map<Integer, Map<Integer, Map<Long, LongSummaryStatistics>>> stats = new ConcurrentHashMap<>();
     ...
     */
    public TocStanoxClassDelayAnalyzer( Consumer<? super JsonStructure> recordingConsumer,
                                        Consumer<? super JsonStructure> realTimeConsumer )
    {
        super( REPORT_ID, recordingConsumer, realTimeConsumer );
    }

    private int getClass( TrainMovement mvt )
    {
        final String headCode = mvt.getHeadCode();
        ...
        else
        {
            try
            {
                return Integer.parseInt( headCode.substring( 0, 1 ) );
            }
            catch( Exception ex )
            {
                return -1;
    ...
    protected LongSummaryStatistics getStatistic( TrainMovement mvt )
    {
        ...
        return stats.computeIfAbsent( mvt.getToc_id(), toc -> new ConcurrentHashMap<>() ).
                computeIfAbsent( getClass(mvt), clazz -> new ConcurrentHashMap<>() ).
    ...
    {
        b.add( "class", getClass(mvt) );
        super.addData( b, mvt, v );
    }

    @Override
    protected void recordData()
    {
        stats.forEach( (toc, classes) ->
        {
            if( !classes.isEmpty() )
            {
                classes.forEach( (trainClass, stanoxes) ->
                {
                    if( !stanoxes.isEmpty() )
                    {
                        ...
                        ReportBuilder b = new ReportBuilder().
                                ...
                                setTitle( "Stanox performance by TOC " + toc + " and train class" + trainClass ).
                                setDescription( "This report contains the analysed delays for all train of class "
                                                + trainClass + " at each stanox" ).
                                setRecordPath( "performance/stanox/%d/%d/%d/%d/%d-%d",
                                               today.getYear(),
                                               today.getMonth().
                                               getValue(),
                                               today.getDayOfMonth(),
