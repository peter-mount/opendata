...
/**
 * Wrapper around {@link ResultSet} to provide support for try resources in the Job language.
 * <p>
 * Note: This wrapper, when used from within a script also implements {@link Iterable} so unlike in Java we can use the resultSet inside an enhanced for loop
 * or in a stream - the contents of which would be the ResultSet itself.
 * <p>
 * @author peter
 */
public final class ResultSetWrapper
        implements ResultSet,
                   Iterable<ResultSetMap>
{
    ...
    public ResultSetWrapper( DataSource dataSource )
            throws SQLException
    {
        this.con = new ConnectionWrapper( this, dataSource.getConnection() );
    }

    @SuppressWarnings("LeakingThisInConstructor")
    public ResultSetWrapper( Connection con )
            throws SQLException
    {
        this.con = new ConnectionWrapper( this, con );
    }
    ...
    {
        return new Iterator<ResultSetMap>()
        {
            @Override
    ...
    @Override
    public Spliterator<ResultSetMap> spliterator()
    {
        return rs == null || map == null ? Spliterators.emptySpliterator() : new ResultSetSpliterator<>( rs, r -> map );
    }
    ...
    public <T> T unwrap( Class<T> iface )
            throws SQLException
    {
        if( rs != null && ResultSet.class.isAssignableFrom( iface ) ) {
            return (T) rs;
        }
    ...
    public boolean isWrapperFor(
            ...
            throws SQLException
    {
        return rs != null && (ResultSet.class.isAssignableFrom( iface ) || rs.isWrapperFor( iface ));
    }

    //</editor-fold>
    //<editor-fold defaultstate="collapsed" desc="Delegate to underlying ResultSet">
    @Override
