...
var SignalAreaMap = (function () {
    ...
    SignalAreaMap.plot = function (map) {
        ...
        a = SignalMap.line([], 0, y1, 28.25, y1);
        a = SignalMap.line(a, 0, y2, 28.25, y2);
        // preston pk
        a = SignalMap.line(a, 6.25, y1, 6.5, y2);
        a = SignalMap.line(a, 6.75, y2, 7, y2 + 1);
        a = SignalMap.buffer(a, 5, y2 + 1);

        a = SignalMap.line(a, 5, y2 + 1, 23.75, y2 + 1);
        a = SignalMap.line(a, 23.75, y2 + 1, 24, y2);
        ...
        // Portslade to hove
        a = SignalMap.line(a, 2, y2 + 6, 28.25, y2 + 6);
        a = SignalMap.line(a, 2, y2 + 7, 28.25, y2 + 7);
        ...
        a = SignalMap.line(a, 20, y1 - 1, 28.25, y1 - 1);
        ...
        a = SignalMap.line(a, 20.25, y1 - 2, 28.25, y1 - 2);
        
        // Brighton
        // Plat 4
        a = SignalMap.line(a, 24.5, y2, 24.75, y2 + 1);
        a = SignalMap.line(a, 24.75, y2 + 1, 28.25, y2 + 1);
        // Plat 8
        a = SignalMap.line(a, 24.5, y1 - 2, 24.75, y1 - 3);
        a = SignalMap.line(a, 24.75, y1 - 3, 28.25, y1 - 3);

        // Buffers P3-8
        a = SignalMap.buffer(a, 28.25, y1 - 3);
        a = SignalMap.buffer(a, 28.25, y1 - 2);
        a = SignalMap.buffer(a, 28.25, y1 - 1);
        a = SignalMap.buffer(a, 28.25, y1);
        a = SignalMap.buffer(a, 28.25, y2);
        a = SignalMap.buffer(a, 28.25, y2 + 1);

        // Link to P3 Rear to Hove line
        a = SignalMap.line(a, 26.75, y2 + 1, 26, y2 + 4);
        a = SignalMap.line(a, 24, y2 + 4, 26, y2 + 4);
        a = SignalMap.line(a, 23.5, y2 + 6, 24, y2 + 4);

        // Siding to P2
        a = SignalMap.line(a, 20, y2 + 4, 23.5, y2 + 4);
        a = SignalMap.line(a, 23.5, y2 + 4, 24, y2 + 6);

        // Plat 1 & 2
        a = SignalMap.buffer(a, 28.25, y2 + 6);
        a = SignalMap.buffer(a, 28.25, y2 + 7);
        
        // Siding off P1
        a = SignalMap.line(a, 23.75, y2 + 7, 24, y2 + 8);
        a = SignalMap.line(a, 23.5, y2 + 8, 28.25, y2 + 8);
        a = SignalMap.buffer(a, 28.25, y2 + 8);
        ...
        // Brighton
        map.station(27, y2+4.5, 'Brighton');
        
        map.berthl(26, y1 - 3, 'F448');
        map.berth(27.5, y1 - 3, 'R448');
        map.platform(25.5, y1 - 2.5, 2.5, '8', '7');
        map.berthl(26, y1 - 2, 'F446');
        map.berth(27.5, y1 - 2, 'R446');

        map.berthl(26, y1 - 1, 'F444');
        map.berth(27.5, y1 - 1, 'R444');
        map.platform(25.5, y1 - .5, 2.5, '6', '5');
        map.berthl(26, y1, 'F442');
        map.berth(27.5, y1, 'R442');

        map.berthl(26, y2, 'F440');
        map.berth(27.5, y2, 'R440');
        map.platform(25.5, y2 + .5, 2.5, '4', '3');
        map.berthl(26, y2 + 1, 'F450');
        map.berth(27.5, y2 + 1, 'R450');

        map.berthl(25.25, y2 + 4, '0667');

        map.berthl(25, y2+6, '0665');
        map.berthl(26, y2+6, 'F661');
        map.berth(27.5, y2+6, 'R661');
        map.platform(25.5, y2 + 6.5, 2.5, '2', '1');
        map.berthl(26, y2 + 7, 'F663');
        map.berth(27.5, y2 + 7, 'R663');
        
        map.berthl(25, y2 + 8, '1281');
